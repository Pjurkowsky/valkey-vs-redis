...
Oba mechanizmy mogą działać równocześnie.
...
Redis Cluster to mechanizm partycjonowania danych i zapewniania wysokiej dostępności. Przestrzeń kluczy jest podzielona na 16384 hash slotów. Każdy klucz jest przypisywany do slotu według wzoru:
...
Valkey to otwartoźródłowy system typu in-memory data store, będący forkiem Redis 7.2.4~\cite{valkeyDocumentation}. Projekt został ogłoszony 28 marca 2024 roku pod patronatem Linux Foundation jako bezpośrednia odpowiedź społeczności na zmianę licencji Redis~\cite{linuxFoundationValkey}.
...
\begin{itemize}
    ...
    \item Valkey 8.1 (31 marca 2025) -- dalsze usprawnienia wydajności i stabilności~\cite{valkey81release}.
    \item Valkey 9.0 (21 października 2025) -- atomowa migracja slotów, znaczące przyspieszenie reshardingu~\cite{valkey90release, valkeyAtomicSlotMigration}.
    \item Valkey 9.1 (19 maja 2026) -- poprawki bezpieczeństwa, redukcja skoków opóźnień podczas rehashingu przez inkrementalne zwalnianie stron pamięci, metryka ruchu sieciowego magistrali klastra~\cite{valkey91release}.
\end{itemize}
...
Kubernetes to system orkiestracji kontenerów, który automatyzuje wdrażanie, skalowanie i zarządzanie aplikacjami kontenerowymi~\cite{k8sDocumentation}. W kontekście niniejszej pracy Kubernetes stanowi środowisko uruchomieniowe dla klastrów Valkey i Redis 7.2.

\subsection{StatefulSet}

StatefulSet to obiekt Kubernetes zaprojektowany do zarządzania aplikacjami stanowymi. Zapewnia:
...
W modelu self-hosted administrator przejmuje pełną odpowiedzialność za cykl życia systemu: instalację, konfigurację, monitoring, aktualizacje, skalowanie, backup, odtwarzanie po awarii oraz reagowanie na incydenty. Model ten daje pełną kontrolę nad konfiguracją i potencjalnie niższe koszty infrastruktury, wymaga jednak wiedzy eksperckiej i dedykowanego nakładu pracy.
...
Wewnętrzna implementacja tych operacji nie jest dokumentowana publicznie, co sprawia, że porównanie z rozwiązaniem self-hosted ma częściowo charakter czarnej skrzynki (ang.~\textit{black-box}). Użytkownik obserwuje jedynie czas od wywołania operacji do osiągnięcia stanu gotowości, bez wglądu w poszczególne fazy realizacji.
...
Nishtala i współautorzy opisują architekturę warstwy cache opartej na Memcached w Facebooku, obsługującej ponad miliard żądań na sekundę~\cite{nishtala2013}. Ich praca dokumentuje problemy widoczne dopiero w dużej skali: kaskadowe awarie po utracie fragmentu warstwy cache, efekt thundering herd po unieważnieniu popularnych kluczy oraz wyzwania związane ze spójnością cache między datacenter. Choć praca dotyczy Memcached, a nie Redis, opisane wzorce obciążeń i mechanizmy awaryjne są reprezentatywne dla systemów in-memory cache i stanowią punkt odniesienia dla scenariuszy failover i presji pamięci badanych w niniejszej pracy.
