...
Mierzone metryki obejmują: przepustowość w oknie bazowym (30~s przed chaos), przepustowość w oknie chaos (60~s trwania \texttt{PodChaos}), procentowy spadek przepustowości, czas degradacji poniżej 50\% poziomu bazowego, szacunkową liczbę utraconych operacji, szczytowe opóźnienie p99 podczas failoveru, liczbę błędów połączenia i odpowiedzi \texttt{CLUSTERDOWN}.

Powyższa procedura wstrzykuje \emph{nagłą} awarię mastera i może być zastosowana wyłącznie do wariantów self-hosted, ponieważ wymaga dostępu do podów. Dla Memorystore for Redis Cluster wykonano osobną obserwację typu black-box odpowiadającą failoverowi \emph{planowanemu}. Usługa nie udostępnia komendy ręcznego failoveru pojedynczego sharda, natomiast pozwala wymusić skoordynowany failover wszystkich węzłów poprzez symulację zdarzenia konserwacji (\texttt{gcloud redis clusters update INSTANCE\_ID --region REGION --simulate-maintenance-event}). W tym wariancie ten sam klient \texttt{failover\_client.py} z identyczną konfiguracją obciążenia jest uruchamiany wobec punktu wykrywania (discovery endpoint) instancji, a po fazie stabilizacji skrypt wywołuje symulację konserwacji i rejestruje widoczny czas operacji zarządzanej oraz czas oczekiwania na powrót instancji do stanu gotowości. Należy podkreślić dwie różnice względem testu self-hosted: failover Memorystore jest planowany i bezstratny (replika jest w pełni dosynchronizowana przed promocją), a operacja obejmuje wszystkie shardy, a nie jeden z trzech. Z tego powodu wynik Memorystore jest traktowany jako managed baseline, a nie jako pomiar równoważny nagłej awarii.
...
Failover & Czas degradacji, wykrycia degradacji, utracone operacje, nieudane żądania, odpowiedzi \texttt{CLUSTERDOWN}. \\
Spójność danych & Utracone potwierdzone zapisy, loss rate, minority/majority loss rate, zapisy odrzucone przez klaster. \\
Resilience & Spadek przepustowości [\%], min ops/sec, szczytowe p99, eksmisje, błędy OOM, powrót do baseline. \\
Utrzymywalność & Liczba kroków procedury, liczba komend operatora, czas procedury, error rate klienta kontrolnego. \\
...
Failover mastera   & \cmark & \cmark & \pmark \\
...
Symbole \cmark{} oraz \xmark{} oznaczają odpowiednio przeprowadzenie testu i jego brak. Symbol \pmark{} w wierszu failoveru dla Memorystore oznacza pomiar ograniczony: w usłudze zarządzanej nie można wstrzyknąć nagłej awarii mastera za pomocą Chaos Mesh, ponieważ dostawca nie udostępnia dostępu do węzłów. Memorystore pozwala natomiast wywołać planowany, skoordynowany failover poprzez symulację zdarzenia konserwacji (\texttt{gcloud redis clusters update --simulate-maintenance-event}). Jest to obserwacja typu black-box odpowiadająca failoverowi planowanemu, a nie nagłej awarii, dlatego nie jest bezpośrednio równoważna testowi \texttt{pod-failure} wykonanemu dla wariantów self-hosted i interpretowana jest jako odrębny punkt odniesienia.
