...
Badania obejmują trzy warianty, które odpowiadają trzem scenariuszom rozważanym przez organizacje korzystające dotychczas z Redis 7.2 na licencji BSD-3: pozostanie przy Redis w wersji self-hosted, migracja na Valkey self-hosted lub przejście na usługę zarządzaną. Tabela~\ref{tab:warianty_badawcze} przedstawia przyjęte warianty.
...
Redis 7.2 Cluster w Kubernetes & Punkt odniesienia wydajnościowy. Klaster Redis 7.2.5 z obrazu Bitnami, wdrożony self-hosted w tym samym środowisku GKE, z identyczną topologią i zasobami co Valkey. Ostatnia wersja Redis na licencji BSD-3, reprezentująca stan wyjściowy przed potencjalną migracją. \\
...
Porównanie trzech wariantów wymaga rozróżnienia poziomu kontroli nad środowiskiem. W przypadku Valkey i Redis 7.2 operator zarządza klastrem Kubernetes, konfiguracją Helm oraz zasobami trwałymi wykorzystywanymi przez system. W przypadku Memorystore użytkownik inicjuje operacje przez API lub narzędzia \texttt{gcloud}, natomiast wewnętrzna realizacja tych operacji pozostaje ukryta. Z tego powodu wariant zarządzany jest traktowany jako porównanie typu black-box. Tam, gdzie jest to możliwe, zachowywane są te same parametry obciążenia i podobny rozmiar danych, jednak nie zakłada się pełnej równoważności infrastruktury.

Nie wszystkie testy mogą objąć wszystkie trzy warianty. Testy wykorzystujące Chaos Mesh --- failover mastera, spójność danych podczas partycji sieciowej oraz resilience przy niedoborze zasobów --- mogą być przeprowadzone wyłącznie dla klastrów self-hosted (Valkey i Redis 7.2), ponieważ Memorystore jest usługą zarządzaną bez bezpośredniego dostępu do podów i infrastruktury Kubernetes. Tabelę~\ref{tab:testy_warianty} w dalszej części rozdziału zestawia zakres testów dla każdego wariantu.
...
Aby zapewnić porównywalność, klastry self-hosted w testach z udziałem Memorystore były uruchamiane z plikiem wartości \texttt{values-normalized-ms-small.yaml}, który ustawia limity i requesty zasobów na 2~vCPU i 7~GiB pamięci RAM oraz parametr \texttt{maxmemory} na wartość 5\,583\,457\,484~B ($\approx$5{,}2~GiB), odpowiadającą faktycznej dostępnej pamięci Memorystore na typie \texttt{redis-standard-small}. Zawartość tego pliku przedstawiono w listingu~\ref{lst:normalized-values}.
...
Testy resilience sprawdzają zachowanie klastra przy presji na CPU i pamięć. Zakłócenia są wprowadzane za pomocą zasobu \texttt{StressChaos} z Chaos Mesh. Tabela~\ref{tab:scenariusze_resilience} przedstawia trzy badane scenariusze.

\begin{table}[H]
\centering
\caption{Scenariusze testów resilience}
\label{tab:scenariusze_resilience}
\begin{tabularx}{\textwidth}{p{3.2cm}p{2.4cm}Xr}
\toprule
\textbf{Scenariusz} & \textbf{Typ} & \textbf{Parametry stresu} & \textbf{Czas} \\
\midrule
CPU stress & \texttt{StressChaos} & 4~workers, 100\% CPU load, 1~pod & 30 s \\
Memory stress & \texttt{StressChaos} & 1~worker, 900~MB, 1~pod & 60 s \\
Memory extreme & \texttt{StressChaos} & 2~workers, 1800~MB łącznie, 1~pod & 60 s \\
\bottomrule
\end{tabularx}
\end{table}

Obciążenie tła jest zgodne z profilem z tabeli~\ref{tab:profil_operacyjny}.

Procedura każdego powtórzenia przebiega następująco:

\begin{enumerate}
    \item uruchomienie poda z \texttt{memtier\_benchmark};
    \item stabilizacja obciążenia;
    \item zastosowanie odpowiedniego manifestu \texttt{StressChaos} --- stres trwa 30~s (CPU) lub 60~s (memory);
    \item po zakończeniu stresu \texttt{memtier\_benchmark} kontynuuje do końca okna 120~s;
    \item zapis wyników JSON i podsumowania.
\end{enumerate}

Dla testów pamięci szczególnie istotny jest wariant warm cache, czyli wcześniejsze wypełnienie klastra danymi. Bez danych test nie oddaje rzeczywistego zachowania systemu, ponieważ presja pamięci może nie uruchomić mechanizmów eksmisji ani nie doprowadzić do sytuacji bliskiej OOM Kill.

Mierzone metryki obejmują: maksymalny procentowy spadek ops/sec względem poziomu bazowego (\texttt{ops\_drop\_pct}), czas degradacji poniżej 80\% poziomu bazowego (\texttt{degradation\_duration\_ms}), maksymalne opóźnienie p99 podczas stresu, informację o powrocie do co najmniej 90\% poziomu bazowego po zakończeniu stresu (\texttt{recovery\_detected}) oraz wykrycie restartu poda (\texttt{pod\_restart\_detected}).
...
Test backupu i odtwarzania obejmuje wypełnienie klastra danymi, utworzenie kopii zapasowej oraz pełne odtworzenie klastra z tej kopii. Procedura jest wykonywana pięciokrotnie dla każdego wariantu, aby uzyskać powtarzalne wyniki. Mierzone są czas poszczególnych faz operacji, całkowity czas niedostępności klastra oraz integralność danych po odtworzeniu.

Dla klastra Valkey w Kubernetes przyjęto metodę opartą o snapshoty wolumenów GCE Persistent Disk. Każde powtórzenie rozpoczyna się od wypełnienia klastra danymi testowymi o zadanym rozmiarze na shard. Następnie skrypt wykonuje \texttt{CLUSTER SAVECONFIG} i \texttt{BGSAVE} na wszystkich węzłach klastra, czekając aż operacja zapisu RDB zakończy się na każdym masterze i replice. Po zakończeniu zapisu StatefulSet jest skalowany do zera replik, co zapewnia spójny stan plików na dyskach. Skrypt tworzy następnie snapshot GCE Persistent Disk dla każdego z sześciu wolumenów PVC klastra. Po utworzeniu snapshotów rejestrowany jest manifest zawierający nazwy dysków źródłowych, nazwy snapshotów, klasę storage i rozmiar wolumenu.

Faza odtwarzania polega na przywróceniu dysków ze snapshotów, odtworzeniu PVC z tych dysków, a następnie uruchomieniu StatefulSetu wskazującego na odtworzone wolumeny. Skrypt czeka na osiągnięcie stanu \texttt{cluster\_state:ok} z co najmniej trzema masterami. Po osiągnięciu gotowości uruchamiany jest pod weryfikacyjny, który sprawdza integralność próbki danych porównując wartości kluczy z raportem seedowania. Mierzone czasy obejmują: czas BGSAVE, czas skalowania w dół, czas tworzenia snapshotów, czas odtwarzania dysków, czas startu podów po odtworzeniu oraz czas osiągnięcia gotowości klastra.

Dla usługi Memorystore procedura wykorzystuje zarządzany mechanizm backupu dostawcy chmurowego. Po wypełnieniu klastra danymi testowymi skrypt tworzy kopię zapasową poleceniem \texttt{gcloud redis clusters create-backup}. Następnie odtworzenie realizowane jest przez utworzenie nowego klastra z parametrem \texttt{--import-managed-backup}, wskazującym zasób backupu. Skrypt czeka na osiągnięcie stanu gotowości nowego klastra, po czym uruchamia pod weryfikacyjny łączący się z odtworzonym klastrem i sprawdzający integralność danych. Po zakończeniu weryfikacji klaster odtworzeniowy jest usuwany. Wariant zarządzany nie pozwala rozdzielić wewnętrznych faz operacji, dlatego w wynikach prezentowany jest łączny czas od wywołania komendy tworzenia backupu do osiągnięcia gotowości oraz łączny czas od utworzenia klastra z backupu do jego pełnej dostępności.
...
Rolling upgrade & tak & --- & --- \\
Backup i restore & tak (PVC snapshots) & --- & tak (managed backup) \\
