...
Runs an online Redis 7.2 backup/restore benchmark without kubectl cp for backup
artifacts:
  1. create a fresh Redis 7.2 cluster,
...
Environment:
  ...
  NS=redis
  RELEASE=redis72
  STS=redis72-redis-cluster
  PROJECT_ID=<gcloud active project>
  LOCATION=europe-central2
  ZONE=europe-central2-a
  CHART_PATH=oci://registry-1.docker.io/bitnamicharts/redis-cluster
  VALUES_FILE=k8s/manifests/values-redis72.yaml
  BACKUP_IMAGE=<LOCATION>-docker.pkg.dev/<PROJECT_ID>/valkey-bench/backup_restore:1
  COPY_IMAGE=busybox:1.36
  REDIS_CONTAINER=<statefulset name>
  REDIS_DATA_DIR=/bitnami/redis/data
  RDB_RELATIVE_PATH=dump.rdb
  RANDOM_DATA=true
  VERIFY_MODE=size
...
LOCAL_OUT="${2:-./results/redis72_online_replica_snapshot}"
N="${N:-1}"
...
CHART_PATH="${CHART_PATH:-oci://registry-1.docker.io/bitnamicharts/redis-cluster}"
...
RESTORE_DISK_TYPE="${RESTORE_DISK_TYPE:-pd-ssd}"
EXPECTED_SHARDS="${EXPECTED_SHARDS:-3}"
TARGET_REPLICAS="${TARGET_REPLICAS:-6}"
REDIS_CONTAINER="${REDIS_CONTAINER:-${STS}}"
REDIS_DATA_DIR="${REDIS_DATA_DIR:-/bitnami/redis/data}"
RDB_RELATIVE_PATH="${RDB_RELATIVE_PATH:-dump.rdb}"
RANDOM_DATA="${RANDOM_DATA:-true}"
VERIFY_MODE="${VERIFY_MODE:-size}"
PARALLEL_SHARD_OPS="${PARALLEL_SHARD_OPS:-true}"
REMOTE_OUT="/work/results/redis72_online_replica_snapshot"
...
POD_SELECTOR="app.kubernetes.io/instance=${RELEASE},app.kubernetes.io/name=redis-cluster"
PVC_SELECTOR="${PVC_SELECTOR:-${POD_SELECTOR}}"
...
redis_exec() {
  ...
  if [[ -n "${REDIS_CONTAINER}" ]]; then
    kubectl exec "${pod_name}" -c "${REDIS_CONTAINER}" -n "${NS}" -- "$@"
  else
...
wait_for_no_redis_pods() {
  ...
  echo "ERROR: Redis pods did not terminate within ${timeout_s}s." >&2
...
wait_cluster_healthy() {
  ...
  while [[ ${elapsed} -lt ${max_wait} ]]; do
    ...
    state="$(redis_exec "${ADMIN_POD}" redis-cli cluster info 2>/dev/null | grep cluster_state | tr -d '[:space:]')" || true
    slots="$(redis_exec "${ADMIN_POD}" redis-cli cluster info 2>/dev/null | grep cluster_slots_ok | cut -d: -f2 | tr -d '[:space:]\r')" || true
    masters="$(redis_exec "${ADMIN_POD}" redis-cli cluster nodes 2>/dev/null | grep master | grep -v fail | wc -l | tr -d '[:space:]')" || true
  ...
  redis_exec "${ADMIN_POD}" redis-cli cluster info || true
  redis_exec "${ADMIN_POD}" redis-cli cluster nodes || true
  return 1
}

cluster_nodes_raw() {
  redis_exec "${ADMIN_POD}" redis-cli cluster nodes 2>/dev/null
}
...
write_backup_plan() {
  ...
  "${PYTHON_BIN}" - "${nodes_file}" "${pods_file}" "${STS}" "${EXPECTED_SHARDS}" > "${plan_file}" <<'PY'
import json
import sys

nodes_path, pods_path, pod_prefix, expected_shards = sys.argv[1:5]
...
def endpoint_to_pod(endpoint: str) -> str:
    if "," in endpoint:
        ...
        if pod.startswith(f"{pod_prefix}-"):
            return pod
    endpoint = endpoint.split("@", 1)[0]
    host = endpoint.rsplit(":", 1)[0]
    if host in ip_to_pod:
        return ip_to_pod[host]
    if host.startswith(f"{pod_prefix}-"):
...
pvc_for_pod() {
  ...
  local pod_json
  pod_json="$(mktemp)"
  kubectl get pod "${pod_name}" -n "${NS}" -o json > "${pod_json}"

  "${PYTHON_BIN}" - "${pod_json}" "${REDIS_CONTAINER}" "${REDIS_DATA_DIR}" <<'PY'
import json
import sys

pod_path, container_name, data_dir = sys.argv[1:4]
...
if container is None:
    raise SystemExit("pod has no containers")

mounts = container.get("volumeMounts", [])
selected = None
for mount in mounts:
    mount_path = mount.get("mountPath", "").rstrip("/") or "/"
    if data_dir == mount_path or data_dir.startswith(mount_path + "/"):
        if selected is None or len(mount_path) > len(selected.get("mountPath", "")):
            selected = mount

if selected is None:
    names = ", ".join(m.get("mountPath", "") for m in mounts)
    raise SystemExit(f"cannot find PVC mount for data dir {data_dir}; mounts: {names}")

volume_name = selected.get("name")
for volume in pod.get("spec", {}).get("volumes", []):
    if volume.get("name") == volume_name:
        pvc = volume.get("persistentVolumeClaim", {}).get("claimName")
        if pvc:
            print(pvc)
            raise SystemExit(0)

raise SystemExit(f"mount {volume_name} is not backed by a PVC")
PY
  rm -f "${pod_json}"
}

write_target_pvc_map() {
  local plan_file="$1"
  local output_file="$2"

  : > "${output_file}"
  while IFS=$'\t' read -r master_id master_pod replica_pod slots; do
    local master_pvc replica_pvc
    master_pvc="$(pvc_for_pod "${master_pod}")"
    replica_pvc="$(pvc_for_pod "${replica_pod}")"
    printf '%s\t%s\t%s\t%s\t%s\n' "${master_pod}" "${replica_pod}" "${master_pvc}" "${replica_pvc}" "${slots}" >> "${output_file}"
  done < "${plan_file}"
}

target_master_pvc_from_map() {
  local map_file="$1"
  local master_pod="$2"
  awk -F'\t' -v pod="${master_pod}" '$1 == pod { print $3; found=1 } END { exit found ? 0 : 1 }' "${map_file}"
}

assert_replica_synced() {
  local replica_pod="$1"
  local replication
  replication="$(redis_exec "${replica_pod}" redis-cli info replication 2>/dev/null)" || true
...
trigger_replica_bgsaves() {
  ...
  if [[ "${PARALLEL_SHARD_OPS}" == "true" ]]; then
    ...
    while IFS=$'\t' read -r master_id master_pod replica_pod slots; do
      (
        ...
        redis_exec "${replica_pod}" redis-cli bgsave >/dev/null || true
      ) &
      bgsave_pids+=("$!")
    done < "${plan_file}"
  ...
  while [[ "${all_done}" != "true" && ${waited} -lt ${max_wait} ]]; do
    ...
    while IFS=$'\t' read -r master_id master_pod replica_pod slots; do
      local persistence
      persistence="$(redis_exec "${replica_pod}" redis-cli info persistence 2>/dev/null)" || true
  ...
  if [[ "${PARALLEL_SHARD_OPS}" == "true" ]]; then
    ...
    while IFS=$'\t' read -r master_id master_pod replica_pod slots; do
      (
        ...
        redis_exec "${replica_pod}" redis-check-rdb "${REDIS_DATA_DIR}/${RDB_RELATIVE_PATH}" >/dev/null
      ) &
      validate_pids+=("$!")
    done < "${plan_file}"
...
now = int(time.time())
doc = {
    "variant": "redis72_online_replica_pvc_snapshot",
  ...
  if [[ "${PARALLEL_SHARD_OPS}" == "true" ]]; then
    ...
    while IFS=$'\t' read -r master_id master_pod replica_pod slots; do
      (
        ...
        source_pvc="$(pvc_for_pod "${replica_pod}")"
        target_pvc="$(pvc_for_pod "${master_pod}")"
...
' "${master_pod}" "${replica_pod}" "${source_pvc}" "${target_pvc}" "${source_disk}" "${snapshot}" \
  ...
  cat > "${timing_file}" <<EOF
{
  "variant": "redis72_online_replica_pvc_snapshot_backup",
...
delete_release_and_pvcs() {
  ...
  wait_for_no_redis_pods 300 || true
...
install_fresh_cluster() {
  echo "  Installing fresh Redis 7.2 cluster..."
  kubectl create namespace "${NS}" --dry-run=client -o yaml | kubectl apply -f - >/dev/null
  delete_release_and_pvcs
  helm install "${RELEASE}" "${CHART_PATH}" -n "${NS}" -f "${VALUES_FILE}" --wait=false
  kubectl rollout status sts/"${STS}" -n "${NS}" --timeout=900s
  wait_cluster_healthy 600
}
...
seed_data() {
  ...
  kubectl run "${pod_name}" -n "${NS}" \
    ...
    /bin/sh -c "
      mkdir -p '${REMOTE_OUT}'
      python -u /work/backup_restore_seed.py \
        --mode seed \
        --host '${HOST}' \
        --port '${PORT}' \
  ...
  containers:
    - name: copy
      ...
      command:
        ...
        - |
          set -eu
          rdb='${RDB_RELATIVE_PATH}'
          rdb_dir="\$(dirname "/target/\${rdb}")"
          mkdir -p "\${rdb_dir}"
          rm -f "/target/\${rdb}" /target/appendonly.aof
          rm -rf /target/appendonlydir
          cp "/source/\${rdb}" "/target/\${rdb}"
          chmod 644 "/target/\${rdb}"
    ...
    - name: clear
      ...
      command:
        ...
        - |
          set -eu
          rdb='${RDB_RELATIVE_PATH}'
          rm -f "/data/\${rdb}" /data/appendonly.aof
...
restore_online_replica_snapshots() {
  ...
  local target_pvc_map="${local_out}/target_pvcs.tsv"
  ...
  write_target_pvc_map "${target_plan}" "${target_pvc_map}"
...
' "${master_pod}" "${snapshot}" "${source_pvc}" "${source_pv}" "${source_disk}" "${one_end}" "${one_start}" >> "${restored_jsonl}"
  ...
  if [[ "${PARALLEL_SHARD_OPS}" == "true" ]]; then
    ...
    for encoded in $(manifest_items_base64 "${manifest_file}"); do
      (
        ...
        master_ordinal="${master_pod##*-}"
        source_pvc="${RELEASE}-online-snapshot-source-${master_ordinal}"
        target_pvc="$(target_master_pvc_from_map "${target_pvc_map}" "${master_pod}")"
        copy_pod="online-snapshot-copy-${master_ordinal}"
        echo "  Copying ${source_pvc}:/${RDB_RELATIVE_PATH} -> ${target_pvc}:/${RDB_RELATIVE_PATH}"
  ...
  if [[ "${PARALLEL_SHARD_OPS}" == "true" ]]; then
    local clear_pids=()
    while IFS=$'\t' read -r master_pod replica_pod master_pvc replica_pvc slots; do
      (
        replica_ordinal="${replica_pod##*-}"
        claim_name="${replica_pvc}"
  ...
  kubectl scale sts/"${STS}" -n "${NS}" --replicas="${TARGET_REPLICAS}"
  kubectl rollout status sts/"${STS}" -n "${NS}" --timeout=900s
  local pods_ready
  pods_ready="$(date +%s)"
  ...
  cat > "${timing_file}" <<EOF
{
  "variant": "redis72_online_replica_pvc_snapshot_restore",
  "project_id": $(json_string "${PROJECT_ID}"),
  "zone": $(json_string "${ZONE}"),
  "namespace": $(json_string "${NS}"),
  "release": $(json_string "${RELEASE}"),
...
doc = {
    "variant": "redis72_online_replica_pvc_snapshot_benchmark",
    "run": int(run_idx),
    "run_id": run_id,
...
echo "==> Redis 7.2 online replica PVC snapshot benchmark"
